...
```bash
docker-fio-lambda$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
docker-fio-lambda$ sam local start-api
docker-fio-lambda$ curl http://localhost:3000/
```
...
```bash
docker-fio-lambda$ sam logs -n HelloWorldFunction --stack-name docker-fio-lambda --tail
```
...
```bash
docker-fio-lambda$ cd hello-world
...
```bash
sam delete --stack-name docker-fio-lambda
```
